          ...
          return (
            <section key={cat.id} className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
              ...
              >
                ...
                {doneInCat > 0 && (
                  <span className="text-[10px] bg-volt/15 text-volt px-2 py-0.5 rounded-full font-semibold">{doneInCat} active</span>
                )}
              </button>
                      ...
                      <div key={ex.id} className={active ? "bg-zinc-950 border border-zinc-800 rounded-xl p-3 space-y-2.5" : "py-0.5"}>
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            disabled={active}
                            aria-label={active ? `${ex.name} added` : `Add ${ex.name}`}
                            className={`w-7 h-7 shrink-0 rounded-full text-sm font-bold flex items-center justify-center transition-all active:scale-90 ${active ? "bg-volt text-volt-ink" : "bg-zinc-800 text-zinc-300 hover:bg-zinc-700"}`}
                          >
                            {active ? "✓" : "+"}
                          </button>
                        ...
                        {editing && (
                          <div className="ml-9 space-y-1.5 mt-1">
                              ...
                              <div key={i} className="flex items-center gap-2">
                                ...
                                <div className="flex items-center gap-1"><span className="text-[10px] text-zinc-500">kg</span>
                                  <input type="number" inputMode="decimal" step="0.5" value={s.weight} onChange={(e) => updateBmSet(i, "weight", e.target.value)} placeholder="–"
                                    className="w-16 bg-zinc-900 border border-zinc-700 rounded-lg px-2 py-1.5 text-xs text-white text-center tabular-nums focus:outline-none focus:border-amber-400" /></div>
                                <div className="flex items-center gap-1"><span className="text-[10px] text-zinc-500">reps</span>
                                  <input type="number" inputMode="numeric" value={s.reps} onChange={(e) => updateBmSet(i, "reps", e.target.value)} placeholder="–"
                                    className="w-14 bg-zinc-900 border border-zinc-700 rounded-lg px-2 py-1.5 text-xs text-white text-center tabular-nums focus:outline-none focus:border-amber-400" /></div>
                                <div className="flex items-center gap-1"><span className="text-[10px] text-zinc-500">RPE</span>
                                  <select value={s.rpe} onChange={(e) => updateBmSet(i, "rpe", e.target.value)}
                                    className="w-14 bg-zinc-900 border border-zinc-700 rounded-lg px-1 py-1.5 text-xs text-white focus:outline-none focus:border-amber-400">
                                    <option value="">–</option>
                                    {[6, 6.5, 7, 7.5, 8, 8.5, 9, 9.5, 10].map((v) => (<option key={v} value={v}>{v}</option>))}
                                  </select></div>
                                {bmSets.length > 1 && (
                                  <button onClick={() => setBmSets((prev) => prev.filter((_, j) => j !== i))} aria-label="Remove benchmark set" className="text-[10px] text-zinc-600 hover:text-red-400">✕</button>
                                )}
                              </div>
                            ))}
                            <div className="flex items-center gap-2 pt-0.5">
                              ...
                              <button onClick={() => saveBenchmark(ex.name)} className="px-2.5 py-1.5 bg-amber-400 text-zinc-950 rounded-lg text-[11px] font-bold active:scale-95">Save benchmark</button>
                          ...
                          <>
                            <div className="flex items-center justify-between">
                              <div className="flex items-baseline gap-2 flex-wrap">
                                <span className="text-[10px] text-zinc-600 tabular-nums">{doneCount}/{sets.length} sets</span>
                                {beatBenchmark && <span className="text-[10px] text-emerald-400 font-bold">↑ Beat benchmark! e1RM {curE1RM} vs {bmE1RM}</span>}
                                {bmE1RM > 0 && !beatBenchmark && curE1RM > 0 && <span className="text-[10px] text-zinc-500 tabular-nums">e1RM {curE1RM} / target {bmE1RM}</span>}
                              </div>
                            ...
                            {prev && (
                              <p className="text-[10px] text-zinc-500 tabular-nums">Last ({new Date(prev.date).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}): {prev.sets.map((s) => `${s.weight}×${s.reps}`).join(" · ")}</p>
                            )}
                  ...
                  {addingTo === cat.id ? (
                    <div className="flex items-center gap-1.5 pt-1">
                      <input autoFocus value={newExercise} onChange={(e) => setNewExercise(e.target.value)}
                        ...
                        className="flex-1 bg-zinc-950 border border-zinc-700 rounded-lg px-3 py-1.5 text-sm text-white focus:outline-none focus:border-volt" />
                      <button onClick={() => handleAddCatalogExercise(cat.id)} className="px-3 py-1.5 bg-volt text-volt-ink rounded-lg text-xs font-bold hover:bg-volt-bright">Add</button>
                    ...
                    <button onClick={() => { setAddingTo(cat.id); setNewExercise(""); }}
                      className="w-full py-2 text-xs text-zinc-500 hover:text-volt border border-dashed border-zinc-800 hover:border-volt/40 rounded-lg transition-colors mt-1">
                      + New exercise
                    </button>
                  )}
                </div>
      ...
      <div className="pt-1">
        ...
        {showNotes && (
          <textarea
            ...
            rows={3}
            className="w-full bg-zinc-900 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-white placeholder:text-zinc-600 focus:outline-none focus:border-volt resize-none"
          />
        )}
      </div>
