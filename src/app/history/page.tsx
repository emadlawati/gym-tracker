...
export default async function HistoryPage() {
  const sessions = await prisma.workoutSession.findMany({
    include: {
      ...
      exerciseSets: {
        select: { exerciseName: true, completed: true },
      },
    },
    orderBy: { date: "desc" },
  });
    ...
    <div className="space-y-6 pt-4">
      ...
      {sessions.length === 0 ? (
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-8 text-center">
          ...
          <Link
            href="/"
            className="inline-block mt-3 px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium"
          >
            Start a workout
          </Link>
        </div>
        ...
        Object.entries(groupedByMonth).map(([month, monthSessions]) => (
          <section key={month}>
            ...
            <div className="space-y-1">
              {monthSessions.map((s) => {
                const exerciseNames = [
                  ...new Set(s.exerciseSets.map((es) => es.exerciseName)),
                ];
                return (
                  <Link
                    key={s.id}
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {s.completed ? (
                          <span className="text-[10px] bg-emerald-900/50 text-emerald-400 px-1.5 py-0.5 rounded font-medium">
                            Done
                          </span>
                        ) : (
                          <span className="text-[10px] bg-amber-900/50 text-amber-400 px-1.5 py-0.5 rounded font-medium">
                            In Progress
                          </span>
                        )}
                      </div>
