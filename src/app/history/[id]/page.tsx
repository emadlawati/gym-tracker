  ...
  if (!session) {
    return (
      <div className="pt-20 text-center">
        ...
        <Link href="/history" className="text-indigo-400 text-sm mt-2 inline-block">
          Back to history
        </Link>
      </div>
    );
          ...
          .reduce(
            (best, s) => {
              ...
              return e1rm > best.e1rm
                ? { weight: s.weight, reps: s.reps, e1rm }
                : best;
            },
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-zinc-800">
                    <th className="text-left py-2 px-4 text-xs font-semibold text-zinc-500 uppercase">
                      Set
                    </th>
                    <th className="text-right py-2 px-4 text-xs font-semibold text-zinc-500 uppercase">
                      Weight
                    </th>
                    <th className="text-right py-2 px-4 text-xs font-semibold text-zinc-500 uppercase">
                      Reps
                ...
                <tbody>
                  {sets.map((s) => (
                    <tr key={s.id} className="border-b border-zinc-800/50 last:border-0">
                      ...
                      <td className="py-2.5 px-4 text-right font-medium text-white">
                        {s.weight}kg
                      </td>
                  ...
                  <tr className="bg-zinc-800/30">
                    <td className="py-2 px-4 text-xs text-zinc-500 font-medium" colSpan={4}>
                      Exercise volume
                    </td>
                    <td className="py-2 px-4 text-right text-sm font-semibold text-indigo-400">
                      {exerciseVolume}kg
                    </td>
                  </tr>
                </tbody>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <p className="text-lg font-bold text-white">
              {session.exerciseSets.filter((s) => s.completed).length}
            </p>
          </div>
        </div>
      </div>
